...
// WorkspaceAdapter adapts old reearthx workspace repo to new interface
type WorkspaceAdapter struct {
	inner accountrepo.Workspace
}

func NewWorkspaceAdapter(inner accountrepo.Workspace) accountsRepo.Workspace {
...
func (a *WorkspaceAdapter) Filtered(f accountsRepo.WorkspaceFilter) accountsRepo.Workspace {
	// Convert filter and pass through
	oldFilter := accountrepo.WorkspaceFilter{
		Readable: convertNewWorkspaceIDsToOld(f.Readable),
		Writable: convertNewWorkspaceIDsToOld(f.Writable),
	}
	return &WorkspaceAdapter{
		inner: a.inner.Filtered(oldFilter),
	}
}
...
func (a *WorkspaceAdapter) FindByIDs(ctx context.Context, ids accountsID.WorkspaceIDList) ([]*accountsWorkspace.Workspace, error) {
	oldIDs := convertNewWorkspaceIDsToOld(ids)
...
func (a *WorkspaceAdapter) FindByUserWithPagination(ctx context.Context, uid accountsID.UserID, pagination *usecasex.Pagination) ([]*accountsWorkspace.Workspace, *usecasex.PageInfo, error) {
	oldUID, _ := user.IDFrom(uid.String())
	oldWorkspaces, pageInfo, err := a.inner.FindByUserWithPagination(ctx, oldUID, pagination)
	if err != nil {
		return nil, nil, err
	}
	...
	return newWorkspaces, pageInfo, nil
}
...
func (a *WorkspaceAdapter) FindByIntegration(ctx context.Context, iid accountsID.IntegrationID) ([]*accountsWorkspace.Workspace, error) {
	// Integration not supported in old interface
	return nil, nil
}
...
func convertNewWorkspaceIDsToOld(newIDs accountsID.WorkspaceIDList) workspace.IDList {
	if newIDs == nil {
		return nil
	}
	oldIDs := make(workspace.IDList, 0, len(newIDs))
