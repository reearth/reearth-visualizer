...
type LayerLoader struct {
	...
	cache map[gqlmodel.ID]*gqlmodel.Layer
...
func (l *LayerLoader) Load(key gqlmodel.ID) (*gqlmodel.Layer, error) {
...
func (l *LayerLoader) LoadThunk(key gqlmodel.ID) func() (*gqlmodel.Layer, error) {
	l.mu.Lock()
	if it, ok := l.cache[key]; ok {
		l.mu.Unlock()
		return func() (*gqlmodel.Layer, error) {
			return it, nil
		}
	}
	if l.batch == nil {
...
func (l *LayerLoader) LoadAll(keys []gqlmodel.ID) ([]*gqlmodel.Layer, []error) {
	results := make([]func() (*gqlmodel.Layer, error), len(keys))

	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}

	layers := make([]*gqlmodel.Layer, len(keys))
...
// This method should be used if you want one goroutine to make requests to many
// different data loaders without blocking until the thunk is called.
func (l *LayerLoader) LoadAllThunk(keys []gqlmodel.ID) func() ([]*gqlmodel.Layer, []error) {
	results := make([]func() (*gqlmodel.Layer, error), len(keys))
	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}
...
func (l *LayerLoader) Prime(key gqlmodel.ID, value *gqlmodel.Layer) bool {
	l.mu.Lock()
	var found bool
...
func (l *LayerLoader) Clear(key gqlmodel.ID) {
	l.mu.Lock()
	delete(l.cache, key)
	l.mu.Unlock()
}

func (l *LayerLoader) unsafeSet(key gqlmodel.ID, value *gqlmodel.Layer) {
	if l.cache == nil {
		l.cache = map[gqlmodel.ID]*gqlmodel.Layer{}
	}
	l.cache[key] = value
}
...
func (b *layerLoaderBatch) keyIndex(l *LayerLoader, key gqlmodel.ID) int {
