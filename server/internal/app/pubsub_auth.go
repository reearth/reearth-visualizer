...
//
// If cfg.Config.PubSubPushServiceAccounts is empty, verification is disabled -- this is the
// case for local dev and every existing test, none of which have a real Pub/Sub push
// subscription or Cloud Function signing tokens. Once configured, a request without a valid
// token naming an allowed service account is rejected outright, before anything in the body
// (which is otherwise fully attacker-controlled) is trusted for anything.
