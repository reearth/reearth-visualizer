...
func TestDeleteTeam(t *testing.T) {
	...
	assert.Nil(t, err)
	query := fmt.Sprintf(`mutation { deleteTeam(input: {workspaceId: "%s"}){ workspaceId }}`, wId1)
	request := GraphQLRequest{
		Query: query,
	}
	o := Request(e, uId1.String(), request).Object()
	o.Value("data").Object().Value("deleteTeam").Object().Value("workspaceId").String().IsEqual(wId1.String())

	_, err = r.Workspace.FindByID(context.Background(), wId1)
	assert.Equal(t, rerror.ErrNotFound, err)

	query = fmt.Sprintf(`mutation { deleteTeam(input: {workspaceId: "%s"}){ workspaceId }}`, accountdomain.NewWorkspaceID())
	request = GraphQLRequest{
		Query: query,
	}
	o = Request(e, uId1.String(), request).Object()
...
func TestUpdateTeam(t *testing.T) {
	...
	query := fmt.Sprintf(`mutation { updateTeam(input: {workspaceId: "%s",name: "%s"}){ team{ id name } }}`, wId1, "updated")
	request := GraphQLRequest{
		Query: query,
	}
	o := Request(e, uId1.String(), request).Object()
	...
	query = fmt.Sprintf(`mutation { updateTeam(input: {workspaceId: "%s",name: "%s"}){ team{ id name } }}`, accountdomain.NewWorkspaceID(), "updated")
	request = GraphQLRequest{
		Query: query,
	}
	o = Request(e, uId1.String(), request).Object()
...
func TestAddMemberToTeam(t *testing.T) {
	...
	query := fmt.Sprintf(`mutation { addMemberToTeam(input: {workspaceId: "%s", userId: "%s", role: READER}){ team{ id } }}`, wId1, uId2)
	request := GraphQLRequest{
		Query: query,
	}
...
func TestRemoveMemberFromTeam(t *testing.T) {
	...
	query := fmt.Sprintf(`mutation { removeMemberFromTeam(input: {workspaceId: "%s", userId: "%s"}){ team{ id } }}`, wId2, uId3)
	request := GraphQLRequest{
		Query: query,
	}
	Request(e, uId1.String(), request)
...
func TestUpdateMemberOfTeam(t *testing.T) {
	...
	query := fmt.Sprintf(`mutation { updateMemberOfTeam(input: {workspaceId: "%s", userId: "%s", role: WRITER}){ team{ id } }}`, wId2, uId3)
	request := GraphQLRequest{
		Query: query,
	}
	Request(e, uId1.String(), request)
	...
	query = fmt.Sprintf(`mutation { updateMemberOfTeam(input: {workspaceId: "%s", userId: "%s", role: WRITER}){ team{ id } }}`, accountdomain.NewWorkspaceID(), uId3)
	request = GraphQLRequest{
		Query: query,
	}
	o := Request(e, uId1.String(), request).Object()
