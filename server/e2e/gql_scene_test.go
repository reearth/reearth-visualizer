...
// go test -v -run TestGetScenePlaceholder ./e2e/...

func TestGetScenePlaceholder(t *testing.T) {
	...
	e := StartServer(
		...
		true,
		baseSeeder, // default Seeder (=English)
	)
	...
	for _, group := range r.Value("property").Object().Value("schema").Object().Value("groups").Array().Iter() {
		for _, field := range group.Object().Value("fields").Array().Iter() {
			...
			if fieldId == "tile_type" {
				field.Object().Value("placeholder").Equal("please enter tile type")
			}
			if fieldId == "tile_url" {
				field.Object().Value("placeholder").Equal("please enter tile url")
			}
			if fieldId == "tile_zoomLevel" {
				field.Object().Value("placeholder").Equal("please enter tile zoomLevel")
			}
			if fieldId == "tile_opacity" {
				field.Object().Value("placeholder").Equal("please enter tile opacity")
			}
		}
	}
...
// 			if fieldId == "tile_type" {
// 				field.Object().Value("placeholder").Equal("タイルのタイプを選択")
// 			}
// 			if fieldId == "tile_url" {
// 				field.Object().Value("placeholder").Equal("タイルのURLを入力")
// 			}
// 			if fieldId == "tile_zoomLevel" {
// 				field.Object().Value("placeholder").Equal("ズームレベルを入力")
// 			}
// 			if fieldId == "tile_opacity" {
// 				field.Object().Value("placeholder").Equal("不透明性を入力")
// 			}
// 		}
// 	}
