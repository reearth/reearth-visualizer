...
func TestReservedReearthPrefixScene(t *testing.T) {
	...
	res = publishProject(e, uID, map[string]any{
		"projectId": projectId,
		"alias":     alias.ReservedReearthPrefixScene + projectId,
		"status":    "LIMITED",
	})
	res.Object().IsEqual(map[string]any{
		"id":                projectId,
		"alias":             alias.ReservedReearthPrefixScene + projectId, // ok
		"publishmentStatus": "LIMITED",
	})
...
func TestCheckProjectAlias(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			args: args{alias.ReservedReearthPrefixScene + projectId, projectId},
			want: want{alias.ReservedReearthPrefixScene + projectId, true},
		},
	}
...
func TestCheckProjectAliasError(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			args: args{alias.ReservedReearthPrefixScene + sceneId, ""},
			want: want{
				fmt.Sprintf(
					"Aliases starting with 'c-' or 's-' are not allowed: %s",
					alias.ReservedReearthPrefixScene+sceneId,
				),
			},
		},
		{
