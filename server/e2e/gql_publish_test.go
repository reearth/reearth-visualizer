...
// make e2e-test TEST_NAME=TestDefaultAlias
...
func checkSceneAliasAlreadyExists(res *httpexpect.Value, err *httpexpect.Value) {
	...
	err.Array().IsEqual([]map[string]any{
		{
			"message": "alias is already used in another workspace",
			"path":    []any{"publishProject"},
			"extensions": map[string]any{
...
func TestCheckSceneAliasError(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			args: args{sceneId, ""},
			want: want{"alias is already used in another workspace"},
		},
		{
...
// make e2e-test TEST_NAME=TestCheckStoryAlias
