...
func TestClassify_Upstream(t *testing.T) {
	...
	tests := []struct {
		...
		{"internal message embedding an expected phrase stays a defect", graphql.Errors{{Message: "internal error: dependency not found"}}, ClassUnexpected},
		{"transport request_error stays a defect", graphql.Errors{{Message: "500 Internal Server Error", Extensions: map[string]any{"code": graphql.ErrRequestError}}}, ClassUnexpected},
		{"internal extension stays a defect", graphql.Errors{{Message: "input: createWorkspace not found", Extensions: map[string]any{"internal": map[string]any{"detail": "boom"}}}}, ClassUnexpected},
