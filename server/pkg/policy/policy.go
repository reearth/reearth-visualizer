...
var TechLimited_MaximumSizePerAsset = int64(5 * 1024 * 1024) // 5MB
...
func (p *Policy) EnforceProjectCount(count int) error {
	return p.error(p == nil || p.opts.ProjectCount == nil || *p.opts.ProjectCount >= count || TechLimited_ProjectCount >= count)
}
...
func (p *Policy) EnforcePublishableCount(count int) error {
	return p.error(p == nil || p.opts.PublishableCount == nil || *p.opts.PublishableCount >= count || TechLimited_CustomDomainCount >= count)
}

// Asset Policy
func (p *Policy) EnforceAssetStorageSize(size int64) error {
	return p.error(p == nil || p.opts.AssetStorageSize == nil || *p.opts.AssetStorageSize >= size || TechLimited_AssetStorageSize >= size)
}

func (p *Policy) EnforceMaximumSizePerAsset(size int64) error {
	return p.error(p == nil || p.opts.MaximumSizePerAsset == nil || *p.opts.MaximumSizePerAsset >= size || TechLimited_MaximumSizePerAsset >= size)
}

// Import Policy
func (p *Policy) EnforceProjectImportingTimeout(timeout int) error {
	return p.error(p == nil || p.opts.ProjectImportingTimeout == nil || *p.opts.ProjectImportingTimeout >= timeout || TechLimited_ProjectImportingTimeout >= timeout)
}
...
func (p *Policy) EnforceMaximumProjectExportSize(zipFile afero.File) error {
	...
	if *p.opts.MaximumProjectExportSize >= fileSize {
		return fmt.Errorf("file size (%.2fMB) exceeds %dMB limit", float64(fileSize)/float64(MB), *p.opts.MaximumProjectExportSize)
	}
...
func (p *Policy) EnforceInstallPluginCount(count int) error {
	return p.error(p == nil || p.opts.InstallPluginCount == nil || *p.opts.InstallPluginCount >= count || TechLimited_InstallPluginCount >= count)
}

func (p *Policy) EnforceNLSLayersCount(count int) error {
	return p.error(p == nil || p.opts.NLSLayersCount == nil || *p.opts.NLSLayersCount >= count || TechLimited_NLSLayersCount >= count)
}
