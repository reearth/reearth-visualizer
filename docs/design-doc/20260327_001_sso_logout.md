...
Auth0's `logout()` only destroys the SSO session (cookie) on the current browser. It does NOT revoke refresh tokens stored in other browsers. Therefore:

- **Same browser**: After logout, the SSO session is gone. `getAccessTokenSilently({ cacheMode: "off" })` attempts to use the SSO session, fails, and the user is correctly logged out.
- **Different browser**: The other browser still holds a valid refresh token in localStorage. `getAccessTokenSilently({ cacheMode: "off" })` uses this refresh token to obtain a new access token from the token endpoint, bypassing the SSO session entirely. The user remains logged in.
...
**Design Principle: Sessions are isolated per browser/device**

Sessions are scoped to a browser/device, not to a user account globally. With `useRefreshTokens: true` and `cacheLocation: "localstorage"`, each browser holds its own refresh token in localStorage. Auth0's `logout()` only clears the SSO session cookie and local SDK cache — it does NOT revoke refresh tokens stored in other browsers or devices. Therefore, logging out from one browser does not automatically invalidate sessions in other browsers. Cross-browser logout detection is handled separately via the server-side `latest_logout_at` mechanism described in this document.
...
const getAccessToken = async () => {
  ...
  if (latestLogoutAt !== null) {
    const iat = getJwtIat(token); // decode iat from JWT payload using base64url decoder
...
Verified using a normal browser and a private browser, testing across Dashboard and Visualizer.

### Same browser: SSO session is shared

- Log in to Dashboard and Visualizer in the same browser (same SSO session)
- Log out from Dashboard
- Access Visualizer → `X-Latest-Logout-At` header is detected, `latestLogoutAt > iat` triggers token re-fetch with `cacheMode: "off"`
- Since the Auth0 SSO session has been cleared (same browser), the re-fetch fails and the user is redirected to login
- **Result: Logout is correctly propagated across services within the same browser**

### Different browsers: Sessions are isolated

- Log in to Dashboard in a normal browser and Visualizer in a private browser (separate SSO sessions, separate refresh tokens in localStorage)
- Log out from Dashboard in the normal browser
- Access Visualizer in the private browser → `X-Latest-Logout-At` header is detected, `latestLogoutAt > iat` triggers token re-fetch with `cacheMode: "off"`
- Since the private browser has its own valid refresh token, the re-fetch succeeds with a new token (`iat` > `latestLogoutAt`)
- **Result: Session in the other browser continues — this is expected behavior, as sessions are isolated per browser/device due to independent refresh tokens in localStorage**
