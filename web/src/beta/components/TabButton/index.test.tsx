...
test("1. should be rendered", () => {
  render(<TabButton label="test1" onClick={handleClick} disabled={false} />);
});

test("2. should display button label", () => {
  render(<TabButton label="test2" onClick={handleClick} disabled={false} />);
  expect(screen.getByTestId("atoms-tabbutton")).toBeInTheDocument();
  expect(screen.getByText(/test2/)).toBeInTheDocument();
});

test("3. should disabled true button", () => {
  render(<TabButton label="test3" onClick={handleClick} disabled={true} />);
  expect(screen.getAllByTestId("atoms-tabbutton")[0]).toBeDisabled();
});

test("4. should disabled false button", () => {
  render(<TabButton label="test4" onClick={handleClick} disabled={false} />);
