...
export default function Item<T = unknown, R extends Element = Element>({
  ...
  const draggable =
    !!props.dragItemType && !!props.draggable && !!item.draggable && !!shown;
  const droppable =
    !!props.dragItemType && !!props.droppable && !!item.droppable && !!shown;
  const canDropAtChildren = droppable && !!item.droppableIntoChildren;
  const nextItemDroppable =
    !!parentItem.children?.[index[index.length - 1] + 1]?.droppable;
  ...
  >({
    ...
    canDrop: (i, monitor) => {
      ...
      if (
        ...
        "index" in i &&
        (!i.index.length ||
          arrayEquals(index, i.index) ||
          isAncestor(index, i.index))
      ) {
        return false;
      }
      ...
      const type = getDropType(
        pos,
        canDropAtChildren,
        expanded && !!item.children?.length
      );
      if (
        ((type === "topOfChildren" || type === "bottomOfChildren") &&
          !item.droppable) ||
