...
export default ({
    ...
    WAS_ZONES.forEach((zone) => {
      WAS_SECTIONS.forEach((section) => {
        WAS_AREAS.forEach((area) => {
          overriddenAlignSystem?.[zone]?.[section]?.[area]?.widgets?.forEach(
            (w) => {
  ...
  const onPluginWidgetVisibilityChange = useCallback(
    (widgetId: string, v: boolean) => {
  ...
  const moveWidget = useCallback(
    (widgetId: string, options: WidgetLocationOptions) => {
      if (
