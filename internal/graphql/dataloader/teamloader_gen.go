...
type TeamLoader struct {
	...
	cache map[id.TeamID]*graphql.Team
...
func (l *TeamLoader) Load(key id.TeamID) (*graphql.Team, error) {
...
func (l *TeamLoader) LoadThunk(key id.TeamID) func() (*graphql.Team, error) {
	l.mu.Lock()
	if it, ok := l.cache[key]; ok {
		l.mu.Unlock()
		return func() (*graphql.Team, error) {
			return it, nil
		}
	}
	if l.batch == nil {
...
func (l *TeamLoader) LoadAll(keys []id.TeamID) ([]*graphql.Team, []error) {
	results := make([]func() (*graphql.Team, error), len(keys))

	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}

	teams := make([]*graphql.Team, len(keys))
...
// This method should be used if you want one goroutine to make requests to many
// different data loaders without blocking until the thunk is called.
func (l *TeamLoader) LoadAllThunk(keys []id.TeamID) func() ([]*graphql.Team, []error) {
	results := make([]func() (*graphql.Team, error), len(keys))
	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}
...
func (l *TeamLoader) Prime(key id.TeamID, value *graphql.Team) bool {
	l.mu.Lock()
	var found bool
...
func (l *TeamLoader) unsafeSet(key id.TeamID, value *graphql.Team) {
	if l.cache == nil {
		l.cache = map[id.TeamID]*graphql.Team{}
	}
	l.cache[key] = value
}
