...
type SceneLoader struct {
	...
	cache map[id.SceneID]*graphql.Scene
...
func (l *SceneLoader) Load(key id.SceneID) (*graphql.Scene, error) {
...
func (l *SceneLoader) LoadThunk(key id.SceneID) func() (*graphql.Scene, error) {
	l.mu.Lock()
	if it, ok := l.cache[key]; ok {
		l.mu.Unlock()
		return func() (*graphql.Scene, error) {
			return it, nil
		}
	}
	if l.batch == nil {
...
func (l *SceneLoader) LoadAll(keys []id.SceneID) ([]*graphql.Scene, []error) {
	results := make([]func() (*graphql.Scene, error), len(keys))

	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}

	scenes := make([]*graphql.Scene, len(keys))
...
// This method should be used if you want one goroutine to make requests to many
// different data loaders without blocking until the thunk is called.
func (l *SceneLoader) LoadAllThunk(keys []id.SceneID) func() ([]*graphql.Scene, []error) {
	results := make([]func() (*graphql.Scene, error), len(keys))
	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}
...
func (l *SceneLoader) Prime(key id.SceneID, value *graphql.Scene) bool {
	l.mu.Lock()
	var found bool
...
func (l *SceneLoader) unsafeSet(key id.SceneID, value *graphql.Scene) {
	if l.cache == nil {
		l.cache = map[id.SceneID]*graphql.Scene{}
	}
	l.cache[key] = value
}
