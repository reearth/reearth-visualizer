...
type DatasetLoader struct {
	...
	cache map[id.DatasetID]*graphql.Dataset
...
func (l *DatasetLoader) Load(key id.DatasetID) (*graphql.Dataset, error) {
...
func (l *DatasetLoader) LoadThunk(key id.DatasetID) func() (*graphql.Dataset, error) {
	l.mu.Lock()
	if it, ok := l.cache[key]; ok {
		l.mu.Unlock()
		return func() (*graphql.Dataset, error) {
			return it, nil
		}
	}
	if l.batch == nil {
...
func (l *DatasetLoader) LoadAll(keys []id.DatasetID) ([]*graphql.Dataset, []error) {
	results := make([]func() (*graphql.Dataset, error), len(keys))

	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}

	datasets := make([]*graphql.Dataset, len(keys))
...
// This method should be used if you want one goroutine to make requests to many
// different data loaders without blocking until the thunk is called.
func (l *DatasetLoader) LoadAllThunk(keys []id.DatasetID) func() ([]*graphql.Dataset, []error) {
	results := make([]func() (*graphql.Dataset, error), len(keys))
	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}
...
func (l *DatasetLoader) Prime(key id.DatasetID, value *graphql.Dataset) bool {
	l.mu.Lock()
	var found bool
...
func (l *DatasetLoader) unsafeSet(key id.DatasetID, value *graphql.Dataset) {
	if l.cache == nil {
		l.cache = map[id.DatasetID]*graphql.Dataset{}
	}
	l.cache[key] = value
}
