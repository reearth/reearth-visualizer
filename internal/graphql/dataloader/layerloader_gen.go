...
type LayerLoader struct {
	...
	cache map[id.LayerID]*graphql.Layer
...
func (l *LayerLoader) Load(key id.LayerID) (*graphql.Layer, error) {
...
func (l *LayerLoader) LoadThunk(key id.LayerID) func() (*graphql.Layer, error) {
	l.mu.Lock()
	if it, ok := l.cache[key]; ok {
		l.mu.Unlock()
		return func() (*graphql.Layer, error) {
			return it, nil
		}
	}
	if l.batch == nil {
...
func (l *LayerLoader) LoadAll(keys []id.LayerID) ([]*graphql.Layer, []error) {
	results := make([]func() (*graphql.Layer, error), len(keys))

	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}

	layers := make([]*graphql.Layer, len(keys))
...
// This method should be used if you want one goroutine to make requests to many
// different data loaders without blocking until the thunk is called.
func (l *LayerLoader) LoadAllThunk(keys []id.LayerID) func() ([]*graphql.Layer, []error) {
	results := make([]func() (*graphql.Layer, error), len(keys))
	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}
...
func (l *LayerLoader) Prime(key id.LayerID, value *graphql.Layer) bool {
	l.mu.Lock()
	var found bool
...
func (l *LayerLoader) unsafeSet(key id.LayerID, value *graphql.Layer) {
	if l.cache == nil {
		l.cache = map[id.LayerID]*graphql.Layer{}
	}
	l.cache[key] = value
}
